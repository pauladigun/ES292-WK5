...
We can generate a simulated dataset by directly creating a data frame where each variable is sampled from a normal distribution. Specifically, we will draw 100 values for each of 3 variables. The first variable pred_1 represents our main predictor. The outcome Y will be related only to pred_1. The second predictor pred_2 will also depend on pred_1, inducing a relationship between the predictors. To simplify choosing priors later, we will z-score standardize each variable by applying the scale() function, which centers to mean 0 and scales to standard deviation 1. This simulated data allows us to test modeling where we know the true generative relationships.
...
We can now fit a linear regression on our simulated data to examine the relationship between the outcome Y and the key predictor pred_1. Specifically, we will use a Bayesian model with weakly informative priors on the regression parameters α and β. Because we standardized the variables, we expect the intercept α to be close to 0, so we set an informative Normal(0, 0.5) prior. The slope coefficient β could plausibly range between -2 to 2 given the scales, so we set a wider Normal(0, 2) prior. These priors constrain the parameter space to realistic values, as we saw in prior predictive checks earlier. By fitting this Bayesian regression, we can evaluate how well it captures the true generative model relating pred_1 and Y in our simulated data. Using informative priors aids in regularizing this estimation.
